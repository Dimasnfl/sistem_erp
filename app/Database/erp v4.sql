...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 26 Nov 2022 pada 12.31
...
(2, 'SAP-Fundamental', 'SAP01', 10, 50000),
...
(26, '01/S1', 93, '20 April 2022', 'RAGA AHLIL FIKRI', '0119101072', 'Passed', 'REG A', 'D', 'Proses Cetak', 'Ignatius Oki Dewa Brata, S.E., M.Si., Ak., C.A.', 'ONLINE', 'AC010', NULL, 'Ganjil', 0),
...
(73, '0119101072', NULL, 26, 'AC010', '01/S1', 1, 200000, 1, '2022-11-26'),
(74, '0119101072', 'SAP01', NULL, NULL, NULL, 1, 50000, 1, '2022-11-26');

--
-- Trigger `shopping_cart`
--
DELIMITER $$
CREATE TRIGGER `delete_cart_out` AFTER DELETE ON `shopping_cart` FOR EACH ROW BEGIN
	UPDATE moduls SET ketersediaan = ketersediaan + OLD.qty
...
CREATE TRIGGER `sertifikat_cancel` AFTER DELETE ON `shopping_cart` FOR EACH ROW BEGIN
	UPDATE nilai_sertifikat SET status = "Belum Cetak"  
...
CREATE TRIGGER `sertifikat_out` AFTER UPDATE ON `shopping_cart` FOR EACH ROW BEGIN
	UPDATE nilai_sertifikat SET status = "Sudah Diambil"  
    WHERE nilai_sertifikat.id = OLD.id_sertifikat;
END
$$
DELIMITER ;
